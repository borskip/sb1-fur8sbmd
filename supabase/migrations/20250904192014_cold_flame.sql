/*
  # Fresh Movie Tracker Database Schema
  
  This is a complete, clean database setup for the Movie Tracker application.
  
  ## Tables Created:
  1. users - User profiles linked to Supabase auth
  2. personal_watchlist - Personal movie lists and watchlists
  3. shared_watchlist - Movies shared between users
  4. watched_movies - Movies marked as watched
  5. ratings - Movie ratings (1-5 stars)
  6. tv_watchlist - TV show watchlists
  7. tv_ratings - TV show ratings
  8. recommendations - Movie recommendations between users
...
-- 2. Personal watchlist (both regular list and want-to-see ratings)
...
CREATE POLICY "personal_watchlist_select_all" ON personal_watchlist
  FOR SELECT TO authenticated
  USING (true);

CREATE POLICY "personal_watchlist_insert_own" ON personal_watchlist
  FOR INSERT TO authenticated
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY "personal_watchlist_update_own" ON personal_watchlist
  FOR UPDATE TO authenticated
  USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY "personal_watchlist_delete_own" ON personal_watchlist
...
CREATE POLICY "shared_watchlist_select_all" ON shared_watchlist
  FOR SELECT TO authenticated
  USING (true);

CREATE POLICY "shared_watchlist_insert_auth" ON shared_watchlist
  FOR INSERT TO authenticated
  WITH CHECK (auth.uid() = added_by);

CREATE POLICY "shared_watchlist_update_auth" ON shared_watchlist
  FOR UPDATE TO authenticated
  USING (auth.uid() = added_by);

CREATE POLICY "shared_watchlist_delete_auth" ON shared_watchlist
  FOR DELETE TO authenticated
...
CREATE POLICY "watched_movies_select_all" ON watched_movies
  FOR SELECT TO authenticated
  USING (true);

CREATE POLICY "watched_movies_insert_own" ON watched_movies
  FOR INSERT TO authenticated
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY "watched_movies_update_own" ON watched_movies
  FOR UPDATE TO authenticated
  USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY "watched_movies_delete_own" ON watched_movies
  FOR DELETE TO authenticated
  USING (auth.uid() = user_id);
...
CREATE POLICY "tv_watchlist_select_all" ON tv_watchlist
  FOR SELECT TO authenticated
  USING (true);
...
-- TV ratings policies
CREATE POLICY "tv_ratings_select_all" ON tv_ratings
  FOR SELECT TO authenticated
  USING (true);
...
BEGIN
  ...
  RAISE NOTICE '📊 Tables created: %', (
    ...
    AND table_name IN ('users', 'personal_watchlist', 'shared_watchlist', 'watched_movies', 'ratings', 'tv_watchlist', 'tv_ratings', 'recommendations')
  );
